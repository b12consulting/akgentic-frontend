...
about any message. `agentColours(nodes, palette)` in `core/services/process/selectors/agent-colour.ts`
...
Paths are relative to `src/app/`. The tiers run `core/services/` supplies, `core/components/`
renders, `ui/` assembles — see the *Layout* section of the README. Both components below are in
`core/components/features/`: they are self-contained widgets, and a feature may inject the data
layer, which is why mounting the rating control no longer places `chat-message` in a different tier
from `thinking`.

| File | What it owns |
|---|---|
| `core/services/process/selectors/chat.selector.ts` | the fold; run lifecycle; tool entries; contact absorption |
| `core/services/process/selectors/chat-message.model.ts` | `classifyRule`, `buildLabel`, alignment, fill, collapse, markers |
| `core/services/process/selectors/rateable.ts` | whether a rule is an answer that can be judged |
| `core/services/process/selectors/display-items.ts` | the merge, the sort, run scoping |
| `core/services/process/selectors/day-separator.ts` | the calendar-day arithmetic |
| `core/services/process/selectors/actor-kind.ts` | tool / human / addressable-agent predicates |
| `core/services/process/selectors/agent-colour.ts` | one colour per agent, shared by three surfaces |
| `core/components/features/chat/message/chat-message.component.*` | every rule's rendering |
| `core/components/features/chat/message/thinking.component.*` | the activity fold |

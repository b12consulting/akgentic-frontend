 ...
 *
 * `<app-graph>` is rendered for real rather than stubbed. A schema-ignoring
...
describe('TeamTabsComponent (W6)', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        // `<app-human-request>`, reached through the graph, translates its copy.
  ...
  it('renders the graph', () => {
    expect(host().querySelector('app-graph')).not.toBeNull();
  });
  ...
  it('has the graph as its only child, so nothing else can creep into the pane', () => {
    ...
    expect(children).toEqual(['app-graph']);
  });

  it('keeps the human-request affordance, which the tree also carried', () => {
    // The tree and the graph both rendered `<app-human-request>`; removing the
    ...
    expect(host().querySelector('app-human-request')).not.toBeNull();
  });

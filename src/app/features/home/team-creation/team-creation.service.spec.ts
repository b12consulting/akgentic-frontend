 ...
 *
 * Resolves with a team id, because that is what `ContextService.createTeam`
 * returns now that it reports the new team rather than navigating to it. The id
 * defaults, so the specs that only care about timing say `resolve()`.
 */
function deferred(): {
  ...
  let resolve: (teamId?: string) => void = () => undefined;
  let reject: (e: unknown) => void = () => undefined;
  const promise = new Promise<string>((res, rej) => {
    resolve = (teamId = CREATED_TEAM_ID) => res(teamId);
    reject = rej;
  });
  return { promise, resolve, reject };
}

/** The id the `ContextService` spy reports for a successful create. */
const CREATED_TEAM_ID = 'team-created-1';
...
describe('TeamCreationService', () => {
  ...
  beforeEach(() => {
    ...
    contextSpy.createTeam.and.returnValue(Promise.resolve(CREATED_TEAM_ID));
  ...
  it('(AC3) a gesture on a namespace that asks nothing creates, with no metadata payload', async () => {
    ...
    expect(contextSpy.createTeam.calls.mostRecent().args.length).toBe(2);
    expect(gate.modalVisible).toBeFalse();
  });
  ...
  describe('creatingByGesture (AC6)', () => {
    it('is TRUE while an ungated GESTURE create is in flight, and false once it settles', async () => {
      const post = deferred();
      contextSpy.createTeam.and.returnValue(post.promise);
  ...
  describe('the rejected confirm keeps the dialog open (AC9, AC10)', () => {
    ...
    async function openThenFail(error: unknown): Promise<void> {
      await gate.request(nsSummary('acme-cases', 'Acme Cases', asking), 'gesture');
      contextSpy.createTeam.and.returnValue(Promise.reject(error));
      await gate.confirm({ tenant: 'acme' });
    }
  ...
  // --- created$: the gate reports the team, it does not place it ------------

  describe('created$', () => {
    it('emits the new team id for an UNGATED create', async () => {
      const seen: string[] = [];
      gate.created$.subscribe((id) => seen.push(id));

      await gate.request(nsSummary('agent-team-v1', 'Agent Team'), 'gesture');

      expect(seen).toEqual([CREATED_TEAM_ID]);
    });

    it('emits for the DIALOG path too, so the page answers both the same way', async () => {
      const seen: string[] = [];
      gate.created$.subscribe((id) => seen.push(id));
      ...
      // The path whose promise no caller holds. If this channel were a return
      // value instead, creating through the dialog would land nowhere and the
      // new team would silently not open.
      expect(seen).toEqual([CREATED_TEAM_ID]);
    });

    it('emits for the GESTURE-LESS route', async () => {
      const seen: string[] = [];
      gate.created$.subscribe((id) => seen.push(id));

      await gate.request(nsSummary('agent-team-v1', 'Agent Team'), 'auto');

      expect(seen).toEqual([CREATED_TEAM_ID]);
    });

    it('does NOT emit when the create is refused', async () => {
      const seen: string[] = [];
      gate.created$.subscribe((id) => seen.push(id));
      contextSpy.createTeam.and.returnValue(
        Promise.reject(new HttpError('Unprocessable', 422, 'nope')),
      ...
      // An id for a team the server never made would open an empty pane and
      // put a phantom in the URL.
      expect(outcome).toBe('failed');
      expect(seen).toEqual([]);
    });

    it('emits only AFTER the POST resolves', async () => {
      const seen: string[] = [];
      gate.created$.subscribe((id) => seen.push(id));
      const post = deferred();
      contextSpy.createTeam.and.returnValue(post.promise);
    ...
    it('never routes: the gate still needs no Router at all', async () => {
      // The bed provides no Router. This spec exists so the fix cannot be
      // "undone" by putting the navigation back where it was — doing so would
      // fail to inject here rather than merely changing behaviour.
      await gate.request(nsSummary('agent-team-v1', 'Agent Team'), 'gesture');

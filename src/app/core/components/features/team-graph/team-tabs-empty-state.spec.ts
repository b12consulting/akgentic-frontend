 ...
 *
 * `team-graph.component.spec.ts` already covers this overlay and passes. It
 * cannot see this defect for two reasons, and BOTH have to be removed before
 * the bug appears:
 *
 *  1. IT MOUNTS `TeamGraphComponent` AS THE ROOT OF ITS OWN FIXTURE. In the app the
 *     graph is never a root — `process.component.html` renders
 *     `<app-team-tabs>`, and that is where `<app-team-graph>` lives.
...
describe('TeamTabs → Graph — the empty-state overlay, at production topology', () => {
  ...
  function graph(): TeamGraphComponent {
    return fixture.debugElement.query(By.directive(TeamGraphComponent))
      .componentInstance as TeamGraphComponent;
  }
  ...
  beforeEach(async () => {
    ...
    // `autoDetectChanges`, as in `team-graph.component.spec.ts`: Angular ticks on
  ...
  it('CONTROL — the same data renders correctly once the view is allowed to check', async () => {
    ...
    fixture.debugElement
      .query(By.directive(TeamGraphComponent))

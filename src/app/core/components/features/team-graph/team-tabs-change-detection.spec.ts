 ...
 *
 * `team-graph.component.spec.ts` already pins the overlay's behaviour, and it passes
 * — because it mounts `<app-team-graph>` as the FIXTURE ROOT. A fixture root is
 ...
 * `<app-team-graph>` subtree — on every subsequent pass.
 *
...
describe('The hierarchy pane, mounted the way production mounts it', () => {
  ...
  it('STOPS saying "No agents available" once the agents are on the canvas', async () => {
    ...
    const graph = fixture.debugElement
      .query((de) => de.componentInstance instanceof TeamGraphComponent)

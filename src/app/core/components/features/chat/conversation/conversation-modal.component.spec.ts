...
import {
  ...
  AgentRef,
  ReaderSendRequest,
} from './conversation-modal.component';
...
describe('ConversationModalComponent', () => {
  ...
  describe('activity folds', () => {
    it('shows the open agent’s runs, interleaved chronologically', () => {
      ...
      const rows = Array.from(
        document.querySelectorAll(
          '.conversation-column app-chat-message, .conversation-column app-thinking',
        ),
    ...
    it('shows no other agent’s runs', () => {
      ...
      expect(
        document.querySelectorAll('.conversation-column app-thinking').length,
      ).toBe(0);
    });
